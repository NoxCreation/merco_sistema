...
export default function QuantitySection() {
  ...
  const [data_by_quantity_sponser, setDataByQuantitySponser] = useState([] as Array<OfferRule>)
  const [data_by_quantity_seller, setDataByQuantitySeller] = useState([] as Array<OfferRule>)
  ...
  const onLoad = async () => {
    ...
    await get_configuration({ page: 1, pageSize: 10000, filter }, (status: number, data: any) => {
      if (status == 200) {
        ...
        setDataByQuantitySponser(config.paymentrule.data_by_quantity_sponser)
        setDataByQuantitySeller(config.paymentrule.data_by_quantity_seller)
        setConfig(config)
      }
      else {
  ...
  return (
    <Stack width={"full"} spacing={"20px"} my={"20px"} position={'relative'}>
      ...
      <Stack>
        ...
        <Rules
          isDisabled={true}
          rules={data_by_quantity_sponser}
          onAdd={() => onAdd(3)}
