...
import { create_edit_charge } from "@/helper/requests/Charges";
...
export default function CreateEditChargeDialog({
  action,
  isOpen,
  onClose,
  charge
  ...
  useEffect(() => {
    if (isOpen) {
      if (action == 'edit') {
        setName(charge?.name as string)
        setDescription(charge?.description as string)
      }
      else {
        setName("")
        setDescription("")
  ...
  const onCreateEdit = async () => {
    if (isValid()) {
      ...
      await create_edit_charge(action, charge?.id as number, data, (status: number, data: any) => {
