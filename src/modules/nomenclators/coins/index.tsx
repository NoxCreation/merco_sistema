...
import { get_coin, remove_coin } from "@/helper/requests/Coin";
...
import CreateEditCoinDialog from "./dialog/CreateEditCoinDialog";

export default function NomenclatorsCoinsScreen() {
    ...
    const [items, setItems] = useState([] as Array<Coin>)
    const [itemsSelects, setItemsSelects] = useState([] as Array<Coin>)
    const {
        isOpen,
        onOpen,
        onClose,
    ...
    const onLoad = async (npage?: number, npageSize?: number, new_filter?: {}) => {
        ...
        await get_coin({ page: npage ? npage : pagination.page, pageSize: npageSize ? npageSize : pagination.pageSize, filter }, (status: number, data: any) => {
            if (status == 200) {
                setItems(data.data)
    ...
    const onRemove = async (charge: Coin) => {
        swal({
            title: "¿Está seguro?",
            ...
            .then((willDelete) => {
                if (willDelete) {
                    remove_coin(charge.id, (status: number, data: any) => {
    ...
    return (
        <Box>
            ...
            {/* Tabla */}
            <CRUDTable
                onSelectItems={(products: Array<Coin>) => {
                    if (!isOpen)
